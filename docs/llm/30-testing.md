...
- Postgres-backed handlers: give the handler its dependencies explicitly (`invoices.New(db)`) so tests pass a `nil` pool or a fake, and keep database tests behind `//go:build integration` with `DATABASE_URL`. `postgres.Enable()` in a test app requires `<PREFIX>_POSTGRES_URL` and pings at `Run`, not at `New`.
