...
pkg/lifecycle config log errors health admin middleware httpx httpserver httpclient otel storage
postgres/ jobs/ jwt/ openapi/ web/ feature packages, one module each
providers/<name>/                  third-party service clients (s3, supabase, temporal, workos), one module each, same rules as features
